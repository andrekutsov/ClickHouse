...
Принимает массив, сортирует значения по частоте повторений, удаляет повторения, возвращает массив.
...
- `array` — массив любого типа с произвольным набором элементов типа [Array](../../data_types/array.md).
- `N` — количество возвращаемых значений. Необязательный параметр.

**Возвращаемое значение**

Массив оригинальных значений, в котором значения отсортированы по убыванию частоты повторения (а не по самим значениям).

**Пример**

Передан массив без указания сколько нужно вернуть значений.
...
Передан массив с указанием, что нужно вернуть два самых повторяющихся значения.
